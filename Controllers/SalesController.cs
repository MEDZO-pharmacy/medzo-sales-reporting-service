...
public sealed class SalesController(ISaleService sales):ControllerBase
{
 ...
 {
  try { var result=await sales.CreateAsync(request,ct); return result.AlreadyProcessed?Ok(result):Created($"/api/sales/{result.SaleId}",result); }
  catch(SaleValidationException e){return BadRequest(new{error=e.Message});}
 }
}