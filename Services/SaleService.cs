...
public sealed class SaleService(SalesDbContext db) : ISaleService
{
 public async Task<SaleResponse> CreateAsync(CreateSaleRequest request,CancellationToken ct)
 {
  ...
  if(existing is not null){await transaction.CommitAsync(ct);return ToResponse(existing,true);}
  var sale=new Sale{IdempotencyKey=request.IdempotencyKey};
  foreach(var line in request.Items)
  {
   var batches=await db.StockBatches.Where(x=>x.ProductId==line.ProductId&&x.RemainingQuantity>0&&x.ExpiryDate>=DateOnly.FromDateTime(DateTime.UtcNow)).OrderBy(x=>x.ExpiryDate).ThenBy(x=>x.CreatedAtUtc).ThenBy(x=>x.Id).ToListAsync(ct);
   if(batches.Sum(x=>x.RemainingQuantity)<line.Quantity)throw new SaleValidationException($"Insufficient sellable stock for product '{line.ProductId}'.");
   var item=new SaleItem{ProductId=line.ProductId,Quantity=line.Quantity}; var remaining=line.Quantity;
   foreach(var batch in batches){if(remaining==0)break;var allocated=Math.Min(batch.RemainingQuantity,remaining);batch.RemainingQuantity-=allocated;item.Allocations.Add(new BatchAllocation{StockBatchId=batch.Id,Quantity=allocated});remaining-=allocated;}
   sale.Items.Add(item);
  }
  db.Sales.Add(sale);
  try { await db.SaveChangesAsync(ct); await transaction.CommitAsync(ct); }
  catch(DbUpdateException e) when(IsIdempotencyConflict(e)){await transaction.RollbackAsync(ct);db.ChangeTracker.Clear();var duplicate=await FindByKeyAsync(request.IdempotencyKey,ct);if(duplicate is null)throw;return ToResponse(duplicate,true);}
  return await GetResponseAsync(sale.Id,false,ct);
 }
 static void Validate(CreateSaleRequest r){if(string.IsNullOrWhiteSpace(r.IdempotencyKey))throw new SaleValidationException("An idempotency key is required.");if(r.Items is null||r.Items.Count==0)throw new SaleValidationException("At least one sale item is required.");if(r.Items.Any(x=>string.IsNullOrWhiteSpace(x.ProductId)||x.Quantity<=0))throw new SaleValidationException("Every sale item needs a product and a positive quantity.");if(r.Items.Select(x=>x.ProductId).Distinct(StringComparer.Ordinal).Count()!=r.Items.Count)throw new SaleValidationException("Each product may appear only once per sale.");}
